...
/* hide scrollbar on locked carousel */
...
@media (max-width: 480px){
  ...
  .mp-maker-visual img{width:140px!important;height:140px!important}
      ...
      {/* ── Try it free ── */}
      <section style={{ padding: '80px 40px 60px', background: '#FFFBF5' }} id="try">
        <div style={{ maxWidth: 1000, margin: '0 auto' }}>
          <div className="reveal" style={{ textAlign: 'center', marginBottom: 48 }}>
            <p style={{ fontFamily: "'Nunito',sans-serif", fontSize: 14, fontWeight: 800, letterSpacing: '0.06em', textTransform: 'uppercase', color: '#9A8F82', marginBottom: 12 }}>No sign up needed</p>
            <h2 style={{ fontFamily: "'Nunito',sans-serif", fontWeight: 700, fontSize: 'clamp(28px,4vw,42px)', color: '#2D2A26', letterSpacing: '-0.02em', margin: '0 0 12px' }}>Try a game right now</h2>
            <p style={{ fontFamily: "'Nunito',sans-serif", fontSize: 16, color: '#9A8F82', margin: 0 }}>Jump straight in — three full games, completely free.</p>
          </div>

          {/* 3 demo game cards */}
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(260px, 1fr))', gap: 20, marginBottom: 72 }}>
            {[
              { name: 'Little Shopper', tagline: 'Count coins, budget and buy', path: '/demo/little-shopper', bg: '#2EA820', light: '#E0F0A8', img: '/game%20illustrations/Bank.png', emoji: '💰' },
              { name: 'Little Engineer', tagline: 'Follow wires, flip switches', path: '/demo/little-engineer', bg: '#3A6CE5', light: '#CFD9F4', img: '/game%20illustrations/Bulb.png', emoji: '🔧' },
              { name: 'Little DJ', tagline: 'Tap cells and build a beat', path: '/demo/little-dj', bg: '#CF4A4A', light: '#F2C4BE', img: '/game%20illustrations/Music.png', emoji: '🎛️' },
            ].map((g) => (
              <div key={g.name} className="reveal" style={{ borderRadius: 24, overflow: 'hidden', background: '#fff', border: '1px solid #EDE5D8', boxShadow: '0 4px 20px rgba(0,0,0,0.07)', display: 'flex', flexDirection: 'column' }}>
                {/* Coloured header */}
                <div style={{ background: g.bg, height: 180, display: 'flex', alignItems: 'center', justifyContent: 'center', position: 'relative' }}>
                  <img src={g.img} alt={g.name} style={{ width: 130, height: 130, objectFit: 'contain', filter: 'drop-shadow(0 8px 24px rgba(0,0,0,0.18))' }} />
                  <div style={{ position: 'absolute', top: 14, left: 14, background: 'rgba(255,255,255,0.2)', borderRadius: 9999, padding: '4px 12px', fontFamily: "'Nunito',sans-serif", fontSize: 11, fontWeight: 800, color: '#fff', letterSpacing: '0.06em', textTransform: 'uppercase' }}>Free to play</div>
                </div>
                {/* Body */}
                <div style={{ padding: '20px 20px 24px', display: 'flex', flexDirection: 'column', gap: 10, flex: 1 }}>
                  <div style={{ fontFamily: "'Nunito',sans-serif", fontWeight: 900, fontSize: '1.3rem', color: '#2D2A26', letterSpacing: '-0.02em' }}>{g.name}</div>
                  <div style={{ fontFamily: "'Nunito',sans-serif", fontSize: '0.88rem', color: '#9A8F82', fontWeight: 600, lineHeight: 1.4 }}>{g.tagline}</div>
                  <button
                    onClick={() => { trackLandingClick(`demo_play_${g.name.toLowerCase().replace(/ /g, '-')}`); navigate(g.path); }}
                    style={{ marginTop: 'auto', padding: '12px 0', borderRadius: 9999, border: 'none', background: g.bg, color: '#fff', fontFamily: "'Nunito',sans-serif", fontWeight: 800, fontSize: '0.9rem', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, transition: 'all .2s' }}
                  >
          ...
          <div className="reveal">
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20, flexWrap: 'wrap', gap: 12 }}>
              <div>
                <p style={{ fontFamily: "'Nunito',sans-serif", fontSize: 14, fontWeight: 800, letterSpacing: '0.06em', textTransform: 'uppercase', color: '#9A8F82', margin: '0 0 6px' }}>Members only</p>
                <h3 style={{ fontFamily: "'Nunito',sans-serif", fontWeight: 700, fontSize: 'clamp(20px,3vw,28px)', color: '#2D2A26', margin: 0, letterSpacing: '-0.02em' }}>10 more games waiting for you 🔒</h3>
              </div>
              <button
                onClick={() => { trackLandingClick('locked_carousel_signup'); navigate('/signin'); }}
                style={{ padding: '12px 28px', borderRadius: 9999, border: 'none', background: '#2D2A26', color: '#fff', fontFamily: "'Nunito',sans-serif", fontWeight: 800, fontSize: '0.88rem', cursor: 'pointer', whiteSpace: 'nowrap', flexShrink: 0 }}
              >
            ...
            <div style={{ display: 'flex', gap: 14, overflowX: 'auto', paddingBottom: 12, scrollbarWidth: 'none', WebkitOverflowScrolling: 'touch' }}>
              {[
                { name: 'Little Chef',       emoji: '🍳', color: '#EE6A30', light: '#FDE8D8' },
                { name: 'Little Coder',      emoji: '💻', color: '#EE6A30', light: '#FDE8D8' },
                { name: 'Little Consultant', emoji: '🗂️', color: '#3A6CE5', light: '#CFD9F4' },
                { name: 'Little Trader',     emoji: '🃏', color: '#E8B840', light: '#F0F0A0' },
                { name: 'Little Astronaut',  emoji: '🌟', color: '#E8B840', light: '#F0F0A0' },
                { name: 'Little Chemist',    emoji: '⚗️', color: '#2EA820', light: '#E0F0A8' },
                { name: 'Little Matisse',    emoji: '🎨', color: '#CF4A4A', light: '#F2C4BE' },
                { name: 'Little Pianist',    emoji: '🎹', color: '#3A6CE5', light: '#CFD9F4' },
                { name: 'Little Analyst',    emoji: '🥧', color: '#CF4A4A', light: '#F2C4BE' },
                { name: 'Little Architect',  emoji: '🏗️', color: '#6B8FD8', light: '#CFD9F4' },
              ].map((g) => (
                <div
                  key={g.name}
                  onClick={() => { trackLandingClick('locked_carousel_signup'); navigate('/signin'); }}
                  style={{ flexShrink: 0, width: 160, borderRadius: 20, overflow: 'hidden', background: '#fff', border: '1px solid #EDE5D8', cursor: 'pointer', position: 'relative', userSelect: 'none' }}
                >
                  {/* Card body */}
                  <div style={{ filter: 'blur(1px)', opacity: 0.45 }}>
                    <div style={{ background: g.color, height: 100, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '2.8rem' }}>{g.emoji}</div>
                    <div style={{ padding: '12px 14px 16px' }}>
                      <div style={{ fontFamily: "'Nunito',sans-serif", fontWeight: 900, fontSize: '0.95rem', color: '#2D2A26', marginBottom: 6 }}>{g.name}</div>
                      <div style={{ width: '80%', height: 8, borderRadius: 4, background: '#EDE5D8', marginBottom: 4 }} />
                      <div style={{ width: '60%', height: 8, borderRadius: 4, background: '#EDE5D8' }} />
                    </div>
                  </div>
                  {/* Lock overlay */}
                  <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 6 }}>
                    <div style={{ width: 40, height: 40, borderRadius: '50%', background: '#fff', boxShadow: '0 2px 12px rgba(0,0,0,0.12)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                      <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#2D2A26" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><rect x="3" y="11" width="18" height="11" rx="2" ry="2"/><path d="M7 11V7a5 5 0 0 1 10 0v4"/></svg>
                    </div>
                    <span style={{ fontFamily: "'Nunito',sans-serif", fontWeight: 800, fontSize: '0.7rem', color: '#2D2A26', background: '#fff', borderRadius: 9999, padding: '3px 10px', boxShadow: '0 1px 6px rgba(0,0,0,0.1)' }}>Sign up to play</span>
                  </div>
                </div>
              ))}
            </div>
