...
const KEYFRAMES_ID = 'didit-melody-level-keyframes';
const keyframesCSS = `
...
// song: { id, title, emoji, melody: number[], toast }
export default function MelodyLevel({ song, onMilestone }) {
  const { title, melody } = song;
  ...
  return (
    <div style={{ display: 'flex', flex: 1, width: '100%', alignItems: 'center' }}>
      ...
      >
        ...
        {/* Song title — letters colour in as notes are played */}
        <div style={{
          position: 'absolute',
          top: 14,
          left: 0,
          ...
          {title.split('').map((ch, i) => (
            <span
              ...
              style={{
                color: done
                  ? SOLFEGE[melody[i % melody.length]]?.color ?? 'rgba(255,255,255,0.7)'
                  : i < step
