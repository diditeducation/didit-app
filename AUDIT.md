...
| Price + founding note (one-time $29) | `PRICE` / `PRICE_MODEL` / `PRICE_NOTE` in `src/config.js` | clean ✅ — render the amount via `<Price>` (`<Price />` → "$29", `<Price suffix />` → "$29 one-time"); founding banner via `PRICE_NOTE`. Used by ConversionLanding + Checkout. Never hardcode "$29". Bump `PRICE` + clear `PRICE_NOTE` to end founding pricing. |
