...
**Stripe checkout (scaffolded 2026-06-25):** `/checkout` → "Start playing" calls `startSubscriptionCheckout(uid)` in `src/stripe.js`, which uses the **firestore-stripe-payments Firebase extension** flow: write `customers/{uid}/checkout_sessions` → extension fills in `url` → redirect to Stripe's hosted page. After payment, the extension's webhook writes `customers/{uid}/subscriptions/{id}` (status `active`), which `SubscriptionContext` already listens to → `isMember` flips true. `allow_promotion_codes: true` means Stripe-native promo codes work on the hosted page (a 100%-off coupon = paywall bypass, no custom code). **Gated by `STRIPE_ENABLED`** = `!!VITE_STRIPE_PRICE_ID` (config.js): while unset, the flow is inert and Checkout falls back to dev-simulate (localhost) / "coming soon" notice. **To go live:** (1) install the firestore-stripe-payments extension, (2) create the $15/mo product+price in Stripe, (3) set `VITE_STRIPE_PRICE_ID` in Vercel, (4) update Firestore rules to let a user CREATE `customers/{uid}/checkout_sessions` docs (current rules are read-own only — see §8). **Manage / cancel:** members get a "Manage subscription" item in the **hub profile menu** (gated by `STRIPE_ENABLED && isMember`) → `openCustomerPortal()` calls the extension's `ext-firestore-stripe-payments-createPortalLink` callable (region `VITE_STRIPE_FUNCTIONS_REGION`, default `us-central1`) → Stripe's hosted portal (cancel / update card / invoices), returns to `/hub`. It lives on the hub (auth-gated, **not** paywalled) on purpose, so even a *lapsed* member can always sign in and reach billing — paying members are never locked out, and non-payers have nothing to cancel. **Can't-sign-in escape hatch:** a member who lost access to their login (forgot which email, lost the Google account, etc.) still needs to cancel — so `SiteFooter` shows a **public "Manage subscription" link** (landing / about / hub, no app auth) pointing at Stripe's hosted **Customer Portal *login page*** (`VITE_STRIPE_PORTAL_LOGIN_URL`): the customer enters their email and Stripe emails them a secure manage/cancel link, entirely independent of Did·It auth. Enable + copy that URL from the Stripe Dashboard (Settings → Billing → Customer portal → login link). While the env var is empty the footer link is hidden. Final backstop: you can always cancel anyone from the Stripe Dashboard. **Paywall enforcement (built 2026-06-25, OFF by default):** gated by `PAYWALL_ENFORCED` = `VITE_PAYWALL_ENFORCED === 'true'` (config.js). When ON, non-members are held to the 6 free trial games (`canPlay`/`TRIAL_GAME_IDS`) and locked games route to `/checkout`; enforced at two points — `components/GameRoute.jsx` (route guard on every `/games/*`, redirects locked → checkout) and `GameGrid` (lock badge + "Unlock 🔓" → checkout in the hub). Members & dev/test accounts always pass (`isMember`). **Free tier = the 6 trial games, full version; everything else locked.** ⚠️ **Only flip `VITE_PAYWALL_ENFORCED=true` AFTER Stripe is live** (`STRIPE_ENABLED`), else users get locked with no way to pay — test the locked state first via the dev/test override. While OFF (now), every signed-in user still plays all 12 (no behaviour change).
...
| GameRoute | Auth gate **+ subscription paywall** for `/games/*`. Derives gameId from the URL; when `PAYWALL_ENFORCED`, non-members on a non-trial game → `/checkout?from=<id>`. Off-flag = behaves like ProtectedRoute. | App.jsx (all game routes) |
...
## 9. Analytics
- **Single module:** `src/analytics.js` → flat Firestore `events` collection. One `track*()` per event; never throws (fire-and-forget). Read via Firebase console (no dashboard yet).
- **Every event carries:** `event`, event-specific payload, `userId`/`userEmail` (null when logged-out), **`anonId`** (stable per-browser id — stitches a logged-out visit to the same person after sign-in), **`src`** (first-touch source: `utm_source` / referrer host / `direct`), `env` (`local`/`staging`/`prod`), `date`, `timestamp`.
- **Conversion funnel (landing → paying)** — the ordered events: `page_view{page:'landing'}` → `landing_click{demo_play_*}` → `unlock_click`/`success_click{demo_unlock}` → `signin_view` → `signin_method_click{method}` → (`magic_link_sent`) → `signin_success{method,isNewUser}` → `session_start{isReturn}` (hub) → `checkout_view` → `checkout_start` → `purchase_success` → (`subscription_canceled`). `purchase_success` fires once/account from `SubscriptionContext` when a real sub goes active; `checkout_*` from `Checkout.jsx`; `signin_*` from `SignIn`/`AuthCallback`.
- **Engagement / "what they play & click":** `game_open` / `level_complete` / `game_complete` (per game), `game_home_view`, `share_click`, `filter_select`, `wish_submit`, and `landing_click{buttonId}` (catch-all for clicks — filter by `buttonId`).
- **Known gaps (Phase 2+, not yet built):** concept-card flips untracked; TodayCard fires `game_open` as `"<id>-featured"` (pollutes gameId); no `locked_game_tap`; no time-on-task; `landing_click` mixes landing+hub clicks; no `/admin/analytics` dashboard.
